...
class UserInfoFragment : Fragment()
{
	...
	): View?
	{
		...
		userInfoViewModel.userimage.observe(viewLifecycleOwner, Observer {
			binding.profilePic.setImageBitmap(userInfoViewModel.userimage.value)
		})
		...
		userInfoViewModel.loadResult.observe(viewLifecycleOwner, Observer {
			if (it is Result.Success)
			{
				...
				if (userInfoViewModel.userAdv?.lat != null && userInfoViewModel.userAdv?.logn != null)
				{
					setupLocationText(currLocation);
				}
	...
	private fun setupLocationText(currLocation: TextView)
	{
		val gcd = Geocoder(context)
		val addresses = gcd.getFromLocation(
			userInfoViewModel.userAdv?.lat?.toDouble()!!,
			userInfoViewModel.userAdv?.logn?.toDouble()!!,
			1
		)
		if (addresses.size > 0 && addresses[0].locality != null && addresses[0].postalCode != null)
		{
			currLocation.text = addresses[0].locality + " " + addresses[0].postalCode
		}
		else
		{
			currLocation.text =
				"Unknown0 address at: $userInfoViewModel.userAdv?.locationLat, $userInfoViewModel.userAdv?.locationLog"
		}
	}
